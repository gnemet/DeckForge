...
func main() {
	fmt.Println("=== DeckForge AI Connection Tester ===")
	...
	start := time.Now()
	response, err := client.GenerateContent(ctx, ai.PresentationArchitect, prompt)
	if err != nil {
		fmt.Printf("\n❌ AI ERROR: %v\n", err)
		if active == "gemini" {
	...
	// 5. Output the result
	fmt.Printf("\n✅ AI RESPONSE (%v):\n%s\n", time.Since(start).Round(time.Millisecond), response.Content)
}
